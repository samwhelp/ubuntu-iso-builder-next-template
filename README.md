

# ubuntu-iso-builder-next-template




## Home

| Link | GitHub |
| ---- | ------ |
| [ubuntu-iso-builder-next-template](https://samwhelp.github.io/ubuntu-iso-builder-next-template/) | [GitHub](https://github.com/samwhelp/ubuntu-iso-builder-next-template) |
| [ubuntu-iso-builder-template](https://samwhelp.github.io/ubuntu-iso-builder-template/) | [GitHub](https://github.com/samwhelp/ubuntu-iso-builder-template) |




## Subject

* [Notice](#notice)
* [Docker](#docker)
* [Respin](#respin)
* [Link](#link)




## Notice

* Current default version: `Ubuntu 26.04 (Resolute Raccoon)`
* Currently it still uses `initramfs-tools` and has not switched to `dracut`.




## Docker

| Docker Image |
| ------------ |
| [distro-iso-builder-docker-image](https://github.com/samwhelp/distro-iso-builder-docker-image) |
| [ubuntu-docker-image](https://github.com/samwhelp/ubuntu-docker-image) |




## Respin

| Remix | Respin |
| ----- | ------ |
| [ubuntu-iso-builder-next-remix-xfce](https://github.com/samwhelp/ubuntu-iso-builder-next-remix-xfce) | [ubuntu-iso-builder-next-respin-xfce](https://github.com/samwhelp/ubuntu-iso-builder-next-respin-xfce) |




## Link

| Link | GitHub |
| ---- | ------ |
| [Ubuntu / Adjustment](https://samwhelp.github.io/ubuntu-adjustment/) | [GitHub](https://github.com/samwhelp/ubuntu-adjustment) |
| [Ubuntu / Note](https://samwhelp.github.io/note-about-ubuntu/) | [GitHub](https://github.com/samwhelp/note-about-ubuntu) |
| [debian-iso-builder-template](https://samwhelp.github.io/debian-iso-builder-template/) | [GitHub](https://github.com/samwhelp/debian-iso-builder-template) |




## Samwhelp

* [GitHub](https://github.com/samwhelp)
